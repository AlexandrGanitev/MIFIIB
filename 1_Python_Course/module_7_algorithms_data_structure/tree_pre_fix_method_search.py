...
print("Давайте посмотрим, в каком порядке будет производиться префиксный обход дерева в глубину на примере созданного нами дерева.")
node_root.pre_order()
# Сначала мы записали значение корневого узла (2), после чего — его левого потомка (7). У него также есть левый потомок (2),
# который является листом, поэтому происходит возврат на предыдущий уровень — печатается значение правого потомка (6).
# Дальше просматриваются его потомки и происходит возврат до того уровня, который не просмотрен — правое поддерево корневого узла (/|2|5\).
#
...
print("Постфиксный метод обхода в глубину: ")
